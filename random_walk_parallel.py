...
class GreensFunc:
    ...
    def random_walker(self,initial_position = None, n_walks = None, direction_probs = None):
        ...
        n_walks = n_walks // procs # divides number of walks between processors
        ...
        comm.reduce(self.std_greens_val,op = MPI.SUM, root =0)


        if rank == 0 :
            self.greens_grid /= procs
            self.std_greens_val /=procs
    ...
    def solve(self, initial_position = None, n_walks = None,
        ...
        if np.size(self.x_0) == 1 and np.any(self.x_0) ==0 : #Does the Laplace only version seperately for
            ...
            mc_arr = 1000 # This is the number of values per rank (before
                          # becoming cords of n dimensions)
            #implementing Monte Carlo
            monte_carlo = ParallelMonteCarlo(mc_arr,
                                             [0,int(self.n_grid_points*self.h_space)],
                                             dimensions = 2)
            ...
            comm.reduce(std_phi, op = MPI.SUM, root = 0)
